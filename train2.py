...
if args.swa:
    ...
    swa_res_train = {'loss': None, 'accuracy': None}
...
for epoch in range(start_epoch, args.epochs):
    ...
    if args.swa and (epoch + 1) >= (args.swa_start) and (epoch + 1 - args.swa_start) % args.swa_c_epochs == 0:
        ...
        if epoch == 0 or epoch % args.eval_freq == args.eval_freq - 1 or epoch == args.epochs - 1:
            ...
            swa_res_train = utils.eval(loaders['train'], swa_model, criterion)
        else:
            swa_res = {'loss': None, 'accuracy': None}
            swa_res_train = {'loss': None, 'accuracy': None}
    ...
    if args.swa:
        values = values[:-1] + [swa_res['loss'], swa_res['accuracy']] + [swa_res_train['loss'],
                                                                         swa_res_train['accuracy']] + values[-1:]
...
# python3 train2.py --dir=train164_record_0.1_0.05 --dataset=CIFAR10 --data_path=./../../data --model=PreResNet164 --epochs=225 --lr_init=0.1 --wd=3e-4 --swa --swa_start=126 --swa_lr=0.05 --eval_freq=1 --save_freq=2 --cuda_visible_devices=0